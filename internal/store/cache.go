...
// WatchedEpisodes is every episode number the show counts as seen: finished in
// the player, or at or below the list's progress.
func (s *Store) WatchedEpisodes(ctx context.Context, animeID int) (map[int]bool, error) {
	out := map[int]bool{}
	rows, err := s.r.QueryContext(ctx,
		`SELECT ep_key FROM playback WHERE anime_id = ? AND watched = 1`, animeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
