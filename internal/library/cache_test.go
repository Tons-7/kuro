...
// The list is past it, but it is being rewatched: leaving it must not delete it.
func TestAutoDeleteSparesAnEpisodeGoneBackTo(t *testing.T) {
	c, st := newCache(t)
	ctx := context.Background()
	if err := st.SetSetting(ctx, "cache.autodelete", "keep2"); err != nil {
		t.Fatal(err)
	}
	if _, err := st.MarkWatched(ctx, 7, 6); err != nil {
		t.Fatal(err)
	}
	episode(t, st, "ep1", 1, 7, 1)
	if _, err := st.SavePlayback(ctx, store.PlaybackState{
		AnimeID: 7, EpKey: "1", Position: 30, Duration: 1400, Played: 30,
	}); err != nil {
		t.Fatal(err)
	}

	if n, _ := c.AutoDelete(ctx, 7); n != 0 {
		t.Fatalf("removed %d: the episode being rewatched was deleted", n)
	}
}
